...
| Profile |  Click **Change profile** to select from all available vCPU and RAM combinations.  The profile families are Balanced, Compute, Memory, Ultra High Memory, Very High Memory, Storage Optimized, GPU, Flex, Confidential Compute and High Frequency. For more information, see [Profiles](/docs/vpc?topic=vpc-profiles). |
...
| Boot volume | The default boot volume size for all profiles is 100 GB. You can specify a larger boot volume capacity, up to 250 GB, depending on what the image needs. You can also specify user tags. |
