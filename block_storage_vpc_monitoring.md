...
| Updating | A volume's capacity is [expanding](/docs/vpc?topic=vpc-expanding-block-storage-volumes) or the volume IOPS is getting [adjusted](/docs/vpc?topic=vpc-adjusting-volume-iops). The volume is being migrated to another storage generation's volume profile. | OK |
