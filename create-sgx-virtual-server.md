...
# Creating a virtual server with TDX or SGX
{: #creating-virtual-server-with-confidential-computing-vpc}

[Select availability]{: tag-green}

You can create one or more virtual server instances with TDX or SGX in your {{site.data.keyword.cloud}} VPC.
{: shortdesc}

Confidential computing profiles, Confidential computing with Intel TDX for VPC, and Confidential computing with Intel SGX for VPC are available in the Dallas (us-south), Washington DC (us-east), and Frankfurt (eu-de) regions. For more information, see [Confidential computing known issues](/docs/vpc?topic=vpc-known-issues#confidential-computing-vpc-known-issues). Confidential computing is only available with select profiles. For more information, see [Confidential computing profiles](/docs/vpc?topic=vpc-profiles&interface=ui#confidential-computing-profiles).
{: preview}
...
1. Select an available image. For more information about TDX- or SGX-supported images, see [Limitations](/docs/vpc?topic=vpc-about-confidential-computing-vpc#limitations-confidential-computing-vpc).
1. Select a TDX- or SGX-supported profile by clicking **Confidential computing**. Keep in mind that only the Balanced _bx3dc_ profiles and Compute _cx3dc_ profiles support TDX or SGX.
1. Click **Save**.
1. TDX is the default value. Keep the TDX confidential computing value or select SGX confidential computing.
...
1. Click **Create a virtual server instance** when you are ready to provision. After your new virtual server with TDX or SGX is provisioned, it's ready to use.
