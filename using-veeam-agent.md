...
The [Veeam Agent for Linux](https://www.veeam.com/products/physical/linux-backup-recovery.html){: external} is supported on the Linux&reg; operating systems. You can deploy the Veeam Agent on a virtual server instance for {{site.data.keyword.vpc_full}}, including CentOS, RHEL, Ubuntu, and Debian for instance backups and restoration. Similarly, the [Veeam Agent for Windows](https://www.veeam.com/products/physical/windows-backup-recovery.html){: external} is available for use on Windows instances for {{site.data.keyword.vpc_short}}.
{: shortdesc}
