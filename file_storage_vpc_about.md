...
{{site.data.keyword.filestorage_vpc_full}} delivers NFS-based file storage with zonal and regional availability, encryption, cross-region replication, and multi-VPC sharing for IBM Cloud VPC workloads.
{: shortdesc}

{{site.data.keyword.filestorage_vpc_full}} provides Network File System (NFS)-based file storage services within the VPC Infrastructure. A file share is a type of file storage that is made accessible over the network to allow multiple clients to access the same folders and files simultaneously.
...
- Stunnel - You can establish an encrypted mount connection between the compute host and a **regional** file share by creating a TLS 1.2+ connection between the client virtual server instance and NFS server with stunnel. For more information, see [Encryption in transit - TLS encryption](/docs/vpc?topic=vpc-file-storage-vpc-eit-tls).
...
* Manage your file shares and data.
   ...
   * [Accessing zonal shares from IBM Power Virtual Server instances](/docs/sap?topic=sap-nfs-zonal-powervs-intro){: external}
   * [Accessing regional shares from IBM Power Virtual Server instances](/docs/sap?topic=sap-ha-nlb-rt-rfs-intro){: external}
