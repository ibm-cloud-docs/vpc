...
After you import a custom image to {{site.data.keyword.vpc_short}}, you can view details about the image, manage the lifecycle of the image, use the image to create a virtual server instance, export the image, and even share it with a private catalog. You can also manage custom images that were created from a volume, including updating the image, managing the lifecycle, and deleting it as needed.
{: shortdesc}
...
### Scheduling a lifecycle status change for an image from volume by using the UI
...
### Changing a lifecycle status of an image from volume by using the CLI
...
### Removing a previously scheduled lifecycle status change for an image from volume by using the CLI
...
### Changing the lifecycle status of an image from volume by using the API
...
### Removing a previously scheduled lifecycle status change for an image from volume by using the API
...
### Performance considerations for images from a volume
{: #ifv-performance}
...
#### Canceling an image that is queued for creation
{: #ifv-delete-queued}

You can cancel the image creation by deleting the image if the underlying job didn't start yet. For images in a _pending_ state, the `GET /images` API call returns an `image_request_queued` reason code. You can delete the pending image with a `DELETE /image{image_id}` request. Alternatively, you can use the [UI](#ifv-delete-ui) or [CLI](#ifv-delete-cli) to delete the image that is in a pending state.
