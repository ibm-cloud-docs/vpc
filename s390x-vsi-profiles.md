...
s390x processor architecture profiles can be used to provision IBM Cloud Hyper Protect Virtual Server for {{site.data.keyword.vpc_full}}, LinuxONE virtual server instances, and z/OS virtual server instances. These profiles are available in Japan (Tokyo), Brazil (São Paulo), Canada (Toronto), United Kingdom (London), and US East (Washington DC) regions.
{: preview}
