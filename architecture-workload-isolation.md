...
| {{site.data.keyword.at_full_notm}} | Forwards service audit events to the {{site.data.keyword.at_full}} instance that is set up in the region by the cloud account owner. For more information, see [Activity tracking events for IBM Cloud VPC](/docs/vpc?topic=vpc-at_events). |
