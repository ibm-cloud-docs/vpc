...
| ------- | ------- | -------- |
| [VPN for VPC](/docs/vpc?topic=vpc-vpn-overview) | Secure connectivity over the internet | Hybrid cloud, backup links, rapid deployment |
| [Transit Gateway](/docs/transit-gateway) | VPC-to-VPC routing | Multi-VPC orchestration within IBM Cloud |
| [Direct Link](/docs/dl) | Dedicated private connections from on premises to VPC | High-bandwidth, low-latency private connectivity |
{: caption="Differences between VPN for VPC, Transit Gateway, and Direct Link." caption-side="bottom"}
...
* **Policy-based VPN**: Routes traffic based on defined IP policies, operates in active-standby mode, best for static networks. [Learn more](/docs/vpc?topic=vpc-vpn-overview#policy-based-vpn-gateway).
* **Route-based VPN**: Uses virtual tunnel interfaces and dynamic routing (BGP), supports active-active redundancy, ideal for complex, scalable architectures. [Learn more](/docs/vpc?topic=vpc-vpn-overview#route-based-vpn-gateway).

## What are typical IBM Cloud site-to-site VPN use cases?
{: #faq-vpn-5}
{: faq}

IBM Cloud site-to-site VPN for VPC helps in hybrid cloud extension, backup connectivity, and inter-region VPC communication. See [site-to-site VPN use cases](/docs/vpc?topic=vpc-using-vpn#vpn-use-cases).

## Can site-to-site VPN for VPC serve as a backup connectivity?
{: #faq-vpn-6}
{: faq}

Yes, VPN for VPC is commonly used as a backup to Direct Link or as secondary connectivity alongside Transit Gateway. See [High Availability](/docs/vpc?topic=vpc-vpn-ha).

## How does IBM Cloud site-to-site VPN protect my data?
{: #faq-vpn-7}
{: faq}

VPN for VPC uses advanced **IPsec/IKEv2** encryption algorithms to ensure confidentiality and integrity of data in transit. [Learn more about policy negotiation](/docs/vpc?topic=vpc-using-vpn#policy-negotiation).
...
To troubleshoot IBM Cloud VPN connection issues, check the VPN dashboard tunnel status, validate routes, review access, and firewall rules. For more information, see [site-to-site troubleshooting](/docs/vpc?group=tbs-site-to-site-vpn-gateways). To troubleshoot issues with negotiated, proposed, and received algorithms in IKE and IPsec policies, see [How do I check IPsec logs?](/docs/vpc?topic=vpc-faqs-vpn&interface=ui#faq-vpn-19)
...
To get started with setting up a site-to-site VPN for your VPC, you must create a VPN gateway. See [Creating a VPN Gateway](/docs/vpc?topic=vpc-vpn-create-gateway).

## How many site-to-site VPN connections can I set up per IBM Cloud VPC?
{: #faq-vpn-12}
{: faq}

To determine the number of connections for your site-to-site VPN, see [VPN Quotas](/docs/vpc?topic=vpc-quotas#vpn-quotas).

## Where can I find pricing details for IBM Cloud site-to-site VPN for VPC?
{: #faq-vpn-13}
{: faq}

To find pricing details for IBM Cloud site-to-site VPN for VPC, see the [Pricing tab](/infrastructure/provision/vpngateway).
...
## How are encryption algorithms chosen for IKE and IPsec in a site-to-site VPN connection?
{: #faq-vpn-18}
{: faq}
{: support}

When you select multiple algorithms in an IKE or IPsec policy, the final selection depends on which side chooses the algorithm and the order in which algorithms are evaluated. Differences in ordering can lead to different negotiation outcomes.

* When IBM Cloud initiates the VPN connection, it sends algorithms in the configured order, and the peer makes the final selection. For example, if IBM Cloud proposes `aes128`, `aes256`, and the peer proposes `aes256`, `aes128`, either `aes128` or `aes256` might be selected depending on the peer implementation.
* When IBM Cloud is the responder, it selects the first matching algorithm based on its own configured order. For example, if IBM Cloud is configured with `aes128`, `aes256`, and the peer proposes `aes256`, `aes128`, then `aes128` is selected.

To ensure consistent algorithm selection, configure identical IKE and IPsec algorithms and maintain the same priority order on both the IBM Cloud VPN gateway and the peer gateway.

## How do I check IPsec logs?
{: #faq-vpn-19}
{: faq}
{: support}

To view IPsec logs for negotiated, proposed, and received algorithms, follow these steps:

1. First, configure a logging instance for your VPN instance. For more information, see [Configuring the IBM Cloud Logs Routing service](/docs/logs-router?topic=logs-router-target-cloud-logs).
1. After you set up the logging instance, you can access and review the IPsec log entries that are generated by your VPN. For information about the log format and available fields, see [Logs for VPN for VPC](/docs/vpc?topic=vpc-logging#logs-vpn-vpc).

For any connection debugging, make sure that the proposals that are received from the peer have at least one matching algorithm with the IBM side IKE and IPsec policy. If no matching algorithm exists between the peer and IBM Cloud configuration, the connection fails to establish.
