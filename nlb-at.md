...
The {{site.data.keyword.nlb_full}} (NLB) integrates with {{site.data.keyword.cloudaccesstrailfull_notm}}, which records events in a manner compliant with the CADF standard, as triggered by user-initiated activities that change the state of the service in the cloud.

For a list of actions that are recorded as auditing events on the load balancer service instances, see [{{site.data.keyword.cloudaccesstrailfull_notm}} events](/docs/vpc?topic=vpc-at-events#events-load-balancers).

All auditing events are recorded to the {{site.data.keyword.cloudaccesstrailfull_notm}} in the `us-south` region, regardless of which region the load balancer service is actually provisioned.
{: important}

To view events, you must provision an {{site.data.keyword.cloudaccesstrailfull_notm}} instance in the `us-south` region under your account. Users in your account must have an IAM policy that grants the **Viewer** platform access role and **Reader** service access role on the {{site.data.keyword.cloudaccesstrailfull_notm}} instance.

For more information about granting access, see [{{site.data.keyword.at_short}} events](/docs/vpc?topic=vpc-at-events).

IBM Cloud account users can monitor account-level operations that are performed on the load balancer service.
{: tip}

To provision an {{site.data.keyword.cloudaccesstrailfull_notm}} instance:
