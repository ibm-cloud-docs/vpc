...
| Boot volume size | Specify the size of the boot volume. The default boot volume size for most profiles is 100 GB. Boot volume capacity can be specified in the range of 10 - 250 GB when you're using first-generation volume profiles, and in the range of 10 - 32,000 GB when you're using the `sdp` profile. However, it must match or exceed the size of the image that you're planning to use. Remember that after the volume is provisioned, the size can be increased multiple times. However, it can't be decreased. |
...
| Connectivity | For {{site.data.keyword.cloud}} Hyper Protect Virtual Server for {{site.data.keyword.vpc_full}} instances, make sure that you either [enable a public gateway](/docs/vpc?topic=vpc-about-networking-for-vpc&interface=ui#public-gateway) in the subnet, or [reserve a floating IP address](/docs/vpc?topic=vpc-about-networking-for-vpc&interface=ui#floating-ip) and associate it with the network interface of the instance just after instance creation. |
