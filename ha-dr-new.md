...
| [Fast restore snapshots](/docs/vpc?topic=vpc-snapshots-vpc-faqs&interface=ui#faq-snapshot-fr) | Fast restore snapshots are snapshots that are cached in the zone with the parent block volume. When you create a virtual server with a bootable fast restore snapshot, the server becomes fully operational faster compared to when you provision its boot volume from a normal snapshot. | Fast restore snapshots can achieve a shorter [recovery time objective](#x3167918){: term} (RTO) than restoring from a regular snapshot, but are billed at an extra hourly rate for each zone that the feature is enabled in. |
