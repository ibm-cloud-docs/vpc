...
1. In the [{{site.data.keyword.cloud_notm}} console](/login), go to **menu icon ![menu icon](../icons/icon_hamburger.svg) > VPC Infrastructure > Virtual server instances**.
...
Due to the ephemeral nature of instance storage, configure the mount entry in the Linux&reg; VM’s /etc/fstab file with the `nofail` option, or leave the mount entry out entirely. This avoids a mount failure during boot.
{: important}

Here is an example mount entry with the "nofail" option:
