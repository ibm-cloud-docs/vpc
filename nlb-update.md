...
## Updating a network load balancer in the console
{: #nlb-updating-ui}
{: ui}
...
1. If you are attaching new members, select **Attach** in the Members column of the back-end pool to which you want to add members. Specify the following information, then click **Attach**.
   ...
   * **Member type**: Add virtual server instances, bare metal servers, or an application load balancer (ALB) as a member. If a load balancer is not available, select **Create load balancer**. For virtual server instances, attach each type individually.
