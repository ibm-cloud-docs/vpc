...
1. Begin with a single image file in qcow2 format. You can convert an image to qcow2 format by using the `qemu-img convert` command. 
...
environment by using the [NoCloud ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://cloudinit.readthedocs.io/en/latest/topics/datasources/nocloud.html) data source to supply the metadata.
...
3. Download the [virtio-win drivers](https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/virtio-win.iso). For more information, see the Fedora user documentation, [Creating Windows virtual machines using virtIO drivers](https://docs.fedoraproject.org/en-US/quick-docs/creating-windows-virtual-machines-using-virtio-drivers/).

4. Use VirtualBox to create a virtual machine with the VHD image that you created in step 2. For more information, see [Oracle VM VirtualBox User Manual](https://www.virtualbox.org/manual/). Complete the following steps to customize the virtual machine.
    ...
    2. Start the virtual machine and begin the Windows installation. When you see the page **Where do you want to install Windows?** you must select to load all the drivers in `virtio-win\vioscsi\amd64` and `virtio-win\NetKVM\amd64`. You might need to uncheck "Hide drivers that aren't compatible with this computer's hardware" to access all of the required drivers. Then you can select **Drive 0** and continue with the installation. When the installation is complete, shut down the virtual machine and remove the optical drives that you added previously: Windows installation ISO and virtio-win driver ISO. You can ignore any warnings about removing an optical drive. 
    3. Download [WSUS Offline Update](http://download.wsusoffline.net/wsusoffline118.zip). Run the `wsusoffline\UpdateGenerator`, and then run the `UpdateInstaller.exe`. For more information, see [WSUS Offline Update Documentation](http://www.wsusoffline.net/docs). Select to update the following items: Update C++ Runtime Libraries, Install .NET Framework 3.5" and / or "Install .NET Framework 4.8, Update Root Certificates, Verify installation packages, Automatic reboot and recall.
