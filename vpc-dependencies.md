...
| {{site.data.keyword.at_full_notm}} | The VPC service forwards service audit events to the IBM Cloud Activity Tracker instance that is set up in the region by the cloud account owner. For more information, see [Activity Tracker events](/docs/vpc?topic=vpc-at-events). |
