...
1. Click **New Pool** and specify the following information to create a back-end pool.
   ...
   * Select the method, which is the load-balancing algorithm. The follow options are shown:
      ...
      * **Weighted forwarding** - Forward requests by using an IP hash algorithm to distribute packets across targets based on assigned weights. Each packet is forwarded based on its headers, supporting both symmetric and asymmetric traffic.
...
1. Gather the following information:
   * Your subnet CIDRs (subnets where your workload VSIs reside, which will be the IPs to use as destination IPs).
   * In the load balancer response, you will find two private IP listed. Make note of the first one.
