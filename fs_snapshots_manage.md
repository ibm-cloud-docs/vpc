...
Snapshots that are replicated to a replica share can't be deleted directly from the replica share. They are removed from the replica share at the next replication sync when the source snapshot is deleted on the source share.
{: note}
