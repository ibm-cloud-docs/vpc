...
1. If you're using a policy-based VPN with a static, route-based VPN peer and multiple CIDR subnets on either side, create multiple connections with one CIDR subnet pair per connection.
1. If you're using a Cisco Adaptive Security Appliance (ASA) or Check Point Security Gateway as the peer of {{site.data.keyword.vpn_vpc_short}} with multiple CIDRs and subnets, try moving different subnets to separate connections.
1. If your peer device is Cisco ASAv with a policy-based VPN, the issue might be caused by the device ending the session too early due to its own timeout behavior. In this case, increase the idle‑timeout on the peer or configure it to send periodic keepalive traffic so that the tunnel stays active without requiring long timers.
