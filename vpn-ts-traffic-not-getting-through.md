...
1. If you're using a policy-based VPN with multiple CIDR subnets on either side, create multiple connections with one CIDR subnet pair per connection or look for peer gateway options to allow multiple CIDRs.
...
1. Your peer device might end session too early due to its own timeout behavior. In this case, increase the idle‑timeout on the peer or configure it to send periodic keepalive traffic so that the tunnel stays active without requiring long timers.
