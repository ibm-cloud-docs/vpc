...
| Snapshot request ordering | Only one snapshot creation operation can run at a time per volume. Multiple requests for the same volume may receive a temporary `Volume busy` error before being accepted. Requests for different volumes are accepted independently, but may be queued on the hypervisor for first-generation volumes, so capture times can vary. If you need point-in-time consistency across multiple volumes, consider using a [snapshot consistency group](/docs/vpc?topic=vpc-snapshots-vpc-create-consistency-groups). |
