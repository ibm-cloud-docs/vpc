...
|[Dedicated hosts for VPC](/docs/vpc?topic=vpc-creating-dedicated-hosts-instances) | With dedicated host availability, you can choose to create a single-tenancy environment where you can provision virtual server instances according to your needs.|<burstable>
| [Burstable virtual servers for VPC](/docs/vpc?topic=vpc-burstable-virtual-servers) | When Burstable is enabled, the virtual server is configured to share CPU cores on the host. Which means that multiple virtual servers can use the same CPU resources simultaneously. |</burstable>
| [Spot instances](/docs/vpc?topic=vpc-spot-instances-virtual-servers) | Spot instances are highly discounted versions of the standard instances. They are designed to use available compute resources for interruptible or stateless workloads.  |
