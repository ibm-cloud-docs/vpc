...
In the lifecycle of an operating system, end of support (EOS) is the last date that {{site.data.keyword.cloud}} delivers standard support for a version or release of a product. The end of support date is aligned to the vendor and community support dates. A notification is issued 12 months in advance, with an additional reminder sent one month prior to the withdrawal of support for an operating system.
{: shortdesc}
...
| `deprecated` | Older versions of stock operating systems are `deprecated`. Also, any stock operating system that reached EOS is also `deprecated`. You can still provision an instance with these images. When provisioning a server, the UI displays only `available` images and not `deprecated` images. To provision a server from a `deprecated` image, you must use the CLI or API. |
...
| RHEL AI 3.X (Nvidia) | [RHEL AI Life Cycle](https://access.redhat.com/support/policy/updates/rhelai) | Pay-as-you-Go |
| RHEL AI 3.X (AMD) | [RHEL AI Life Cycle](https://access.redhat.com/support/policy/updates/rhelai) | Pay-as-you-Go |
| RHEL AI 3.X (Intel) | [RHEL AI Life Cycle](https://access.redhat.com/support/policy/updates/rhelai) | Pay-as-you-Go |
