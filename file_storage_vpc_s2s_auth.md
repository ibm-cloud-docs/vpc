...
- Configuring customer-managed encryption:
   ...
   KMS) to hold your CRK. You can choose between {{site.data.keyword.keymanagementserviceshort}} Single or Multi Tenant instances. Then, you need to establish service-to-service authorization between the file service and the KMS instance. The authorization must be created in the account that owns and hosts the customer root key. The account that holds the CRK is the target, and the account where the file share exists is the source.
