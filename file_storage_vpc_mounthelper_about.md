...
You can use Mount Helper to set up encrypted and nonencrypted connections. For encrypted connections, Mount Helper uses the metadata service protocol option that is set to either `http` or `https`. For more information, see the API reference for `metadata_service` option of [instance provisioning](/docs/apis/vpc/latest#create-instance){: external} and [bare metal server provisioning](/docs/apis/vpc/latest#create-bare-metal-server){: external}.
