...
The rules are _stateful_. To have _stateful_ rules means when you create a rule that allows traffic one way, the rule automatically permits reverse traffic. For example, if you create a rule to allow inbound TCP traffic on port 80, the security group also allows replying outbound TCP traffic on port 80 back to the originating host. You don't need to create another rule.
...
When you create a VPC, it is assigned to a default security group. You can modify the default security group settings and the rules of individual security groups.
...
You can't nest security groups within one another, but you can reference remote security groups. Referencing a remote security group in a rule applies that rule to all the reference security group targets attached to that rule.
...
A target is any resource that you attach or detach a security group to or from. Security group targets include virtual server instance network interfaces, endpoint gateways, and load balancers.

Each resource that can send and receive traffic can have a security group that is configured for it. Resources can include virtual server interfaces, bare metal servers, load balancers, any end point gateways along your data path, and more.
...
NACLs are applicable at the subnet level, so any resource in the subnet with an associated NACL follows the rules of the NACL. However, that’s not the case with security groups. Security groups must be assigned explicitly to the resource. Also, unlike NACLs, a security group can be applied to multiple resources across subnets and even across zones.
...
Every security group consists of a set of rules. The security group examines all of its rules before it allows any traffic to enter or leave the instance. The rules that are used to control the inbound traffic are independent of the rules that are used to control the outbound traffic.
...
Because a resource can be associated with multiple security groups, all the rules from each associated security group are combined to form a single set of rules. This set of rules is used to determine whether the traffic is to be denied or allowed into and out from the resource. For every security rule that you add to the security group, you must specify the values for the following fields:
...
A default security group is different from the default rule for new security groups. You can customize both the rule on your default security group, and the default rule that is assigned to new security groups.
{: note}

For example, imagine you are part of an organization and someone with access to your organization's account modifies the default security group's rule. After the modification, the rule is different than the default rule that you get when you create a new security group. Your team member is a very security-conscious customer, and they change the default security group settings to deny all inbound and outbound network traffic.

In this example, when you create a resource, it is initially assigned to your VPC's default security group. This default security group's rule denies all inbound and outbound network traffic because of the deny rules that your security-conscious team member set up. As a result, traffic to and from your new resource has the same deny behavior applied.

If you are discontented with the default security group rule, one solution is to create a different security group with customized settings and apply it to your resource. Another solution is to alter the rule on the default security group. Creating another security group with customized settings is the easiest solution because you can create rules that apply to the single resource that you want to impact. If you alter the rule of the default security group, these changes apply not only to the resource that you want to impact, but to all other resources that are assigned to the default security group.

Any new rules that you create on the default security group are applied to all current and future resources that are assigned to the default security group.
{: important}
...
### Use case 2: Customizing security groups and rules during provisioning
{: #sg-use-case-2}

The following diagram illustrates the potential customization options when you create a new resource. If a User creates a resource and takes no further action to specify a security group to attach it to, their new resource is assigned to the VPC's default security group. This default security group takes on the default security group rule or follow a customized rule that was previously configured by members of your account.

If the User decides to assign the new resource to a new security group, the new security group is automatically assigned to the default security group rule. Then, the User can choose to take no further action, or choose to customize the rules to create a custom security group rule for their new security group. These new rules also apply to the new resource created and assigned to the new security group:
...
A more dynamic way that you can set up your security group is to allow all inbound traffic from other targets that are also part of the security group. When you permit traffic between members of a security group, you don't need to set up individual permissions between each current member and each new member of a security group. When resources are added or deleted from the security group, the permissions apply to all members.

For example, you can have host A and host B in a security group, and you add host C. Normally, you would need to perform maintenance to allow host A and host B to send traffic to and from host C after you add host C to the security group.
...
1. Click **Create +** in the **Inbound rules** table. A **Create inbound rule** window appears.
