...
For {{site.data.keyword.block_storage_is_short}} volumes attached to a virtual server instance, you can increase or decrease IOPS for a volume to meet your performance needs. Adjust IOPs by specifying a different IOPS tier profile or different IOPS value withing a custom IOPS band. There's no outage or lack of access to the storage while adjusting IOPS.
{: shortdesc}
...
For example, you might find that an application has scaled such that a lower-tier storage profile is now a performance bottleneck. Instead of ordering a new volume and migrating your data, change the performance characteristics of the existing volume by increasing IOPS in the next performance tier.

In another scenario, you might want to initially set your storage at a higher 10 IOPS/GB performance level to expedite a data upload. After the upload completes, reset storage to a lower 5 IOPS/GB for normal operations. Alternatively, you might want to increase your volume's IOPS during peak times for your application and decrease IOPS during off-peak hours.

Using this feature, you can:

* Adjust IOPS up or down by selecting a different [IOPS tiers](/docs/vpc?topic=vpc-block-storage-profiles&interface=api#tiers).
...
Increasing IOPS is determined by the maximum allowed by the volume's [IOPS tier profile](/docs/vpc?topic=vpc-block-storage-profiles#tiers). You can adjust IOPS for an IOPS tier based on volume size, or select the next profile that allows for increased performance. 
...
You can use the [UI](#adjust-vpc-iops-ui), [CLI](#adjust-vpc-iops-cli), or [API](#adjust-vpc-iops-api) to adjust IOPS. You can adjust the volume's IOPS multiple times up to its maximum limit, or reduce IOPS to it's lower limit.
...
* If the volume was created using an [IOPS tier profile](/docs/vpc?topic=vpc-block-storage-profiles#tiers), to increase or decrease IOPS, select a different profile for that volume size. If the volume size exceeds that of the new IOPS tier profile, you can't change the profile.
* [Custom IOPS profile](/docs/vpc?topic=vpc-block-storage-profiles#custom) can expand to the highest allowed by the custom band. You can't switch custom bands unless you increase volume size and move to a higher band.
* Custom IOPS can adjusted multiple times until maximum or minimum limit is reached.
...
This example call increases IOPS of a 100 GB volume based on a 100 - 499 custom profile from 100 IOPS to 3,000 IOPS. (The IOPS range for this custom band is 100 - 6,000 IOPS.)
...
You can't update the name of the volume and adjust IOPS in the same `PATCH /volumes` request. Make two separate `PATCH /volumes` requests.
{: note}
...
{
	"iops": 100,
	"created_at": "2021-08-23T09:46:43.000Z",
...
```
curl -X PATCH \
 "$vpc_api_endpoint/v1/volumes/$volume_id?version=2020-08-14&generation=2" \
 -H "Authorization: $iam_token" \
 -d '{
      "profile": "5iops-tier"
    }'
