...
**VPC Metadata service support for bare metal servers.** Accounts that have special approval to preview this feature can now enable access to the VPC Metadata service for bare metal servers when [creating](/apidocs/vpc/latest#create-bare-metal-server) or [updating](/apidocs/vpc/latest#update-bare-metal-server) a bare metal server. You can access the VPC Metadata service on a bare metal server using an [endpoint URL](/apidocs/vpc-identity-beta/latest#endpoint-urls-identity-beta) by specifying the new `metadata_service.protocol` property as `http` or `https`.
