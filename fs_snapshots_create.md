...
Although you can't create a snapshot on a replica share, the snapshots of the origin share are copied from the source to the replica at the next scheduled sync. These replica snapshots are created by the file service. Similarly, when a snapshot is deleted in the source location, it is deleted in the secondary location at the next scheduled synch. Replicated snapshots cannot be manually deleted. They do not inherit the tags or the name from the original snapshots. However, they have the same fingerprint value as the source snapshot.
{: note}
