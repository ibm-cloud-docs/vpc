...
| `peer_not_responding` | `No response from peer (check network ACL configuration, peer availability, and on-premises firewall configuration).` |  There is no response from the peer. Check the network ACL configuration, peer availability, and on-premise firewall configuration. |
| `internal_error` | `Internal error. Contact IBM Support.` |  Contact IBM Support to analyze and resolve internal errors. |
