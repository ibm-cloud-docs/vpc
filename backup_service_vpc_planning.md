...
| Monitoring | Backup jobs that create or delete backup snapshots run according to the backup plan and the retention policy. You can check their status in the console, from the CLI, with the API, or Terraform. If a job fails, the health status code shows the reason for the failure. For more information, see [Viewing backup jobs](/docs/vpc?topic=vpc-backup-view-policy-jobs). [New]{: tag-new} You can also set up a connection to {{site.data.keyword.en_short}} and receive alerts to your preferred destinations. For more information, see [Enabling event notifications for Backup for VPC](/docs/vpc?topic=vpc-event-notifications-events). |
|**Creating backups**: |
