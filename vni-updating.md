...
1. Click **Edit** ![Edit icon](images/edit.png) to change the [protocol state filtering mode](/docs/vpc?topic=vpc-vni-about&interface=ui#protocol-state-filtering), then select one of the following options.
   ...
   * **Enabled**: Forces TCP connections to align with the [RFC793](https://www.rfc-editor.org/rfc/rfc793.txt){: external} standard and any packets to be allowed by corresponding security group rules and network ACLs.
